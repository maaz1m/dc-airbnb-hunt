...
Throughout this paper, we hope to explore the dataset of all listings in a locality such as DC, to determine how property amenities and  location affect the price of an AirBnB listing. 
...
We have longitude and latitude available in our dataset. We plot that according to neighbourhood, with radius of a circle indicating the average price of that location. We can see that as we go eastward, away from the downtown area, the prices tend be lower.
...
```
Of the 116 coefficients, only 27 were found significant for the model. We plot the ten most important features based on magnitude of the coefficients to get a better idea of their contributions to the predictions.
...
Due to the very low R2 value even with a full model, there is no value to be gained by removing insignificant or correlated features. The R2 would decrease with less features. 
