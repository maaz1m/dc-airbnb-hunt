...
We find that we have `r nrow(means_df_select)` neighborhoods to suggest to our friend where the overall AirBnB price is less than DC's average and where the rating is greater than DC's average. We can map these out below. 
...
map <- leaflet(means_df_select) %>%
  ...
  addCircleMarkers(lng = ~longitude,
                   ...
                   radius=~sqrt(Freq),
                   color="#48A9A6",
                   label=~neighbourhood_short,
                   labelOptions = labelOptions(noHide = T, textOnly = TRUE, textsize = "11px"),
                   stroke = FALSE, 
                   fillOpacity = 0.7)
...
ggplot(means_df_rt) + geom_point(aes(x=review_scores_rating, y=price, colour=neighbourhood_short, shape=room_type),size=4, alpha=.85)+  scale_shape_manual(values=c(21,22,23,24))+
  theme(axis.text.x = element_text(angle = 90),
   ...
   geom_vline(xintercept=mean(means_df_rt_eha$review_scores_rating), linetype="dashed", color = "#51c5ed")+ geom_hline(yintercept=mean(means_df_rt_pr$price), linetype="dashed", color = "#C3EB78")+
   geom_vline(xintercept=mean(means_df_rt_pr$review_scores_rating), linetype="dashed", color = "#C3EB78")+ geom_hline(yintercept=mean(means_df_rt_sr$price), linetype="dashed", color = "#ff616f")+
   geom_vline(xintercept=mean(means_df_rt_sr$review_scores_rating), linetype="dashed", color = "#ff616f")+ geom_hline(yintercept=mean(means_df_rt_hr$price), linetype="dashed", color = "#3D348B")+
...
pal <- colorFactor(c("#51c5ed","#C3EB78","#ff616f","#3D348B"), domain = c("Entire home/apt","Shared room","Private room","Hotel room"))
...
map2 <- leaflet(means_df_rt_select) %>%
  ...
  addCircleMarkers(lng = ~longitude,
                   ...
                   color=~pal(room_type),
                   label=~neighbourhood_short,
                   labelOptions = labelOptions(noHide = F, textOnly = T, textsize = "7px"),
                   stroke = FALSE, 
                   fillOpacity = 0.6)%>% 
addLegend("bottomright",pal = pal, values = ~means_df_rt_select$room_type, opacity = 1, title = "Room Type")
