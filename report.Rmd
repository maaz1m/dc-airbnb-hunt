...
Now we will start to look into the neighborhoods in DC. First, we will compare price across the neighborhoods. Before we do this, we should narrow down the neighborhood options for our friend. With our initial dataset, we find there are `r n_distinct(df$neighbourhood)` DC neighborhoods we could recommend our friend look for an AirBnB. We want to make sure that our friend has a large number of AirBnBs to select from our final neighborhood selection. So, we will now only include only neighborhoods with over 175 AirBnB units listed with review and pricing data. 
...
map <- leaflet(means_df_select) %>%
  ...
  addCircleMarkers(lng = ~longitude,
                   ...
                   label=~ID,
                   labelOptions = labelOptions(noHide = T, textOnly = T, textsize = "12px",direction = 'center',style = list(
        "color" = "white")),
                   stroke = FALSE, 
                   fillOpacity = 0.7)%>%
...
```{r}
#anova test to compare mean price
anovaRes_price2 <-  aov(price ~ room_type, data=df_freq)
xkabledply(anovaRes_price2, title = "Price by Room Type ANOVA")

#anova test to compare mean rating
anovaRes_rating2 <-  aov(review_scores_rating ~ room_type, data=df_freq)
xkabledply(anovaRes_rating2, title = "Rating by Room Type Rating ANOVA")
```
...
xkabledply(means_df, title="Subset Neighborhood Means")
