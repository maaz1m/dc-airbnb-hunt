...
To provide greater insights for travelers looking to book an Airbnb in DC, we will be utilizing the DC Airbnb listing data to identify the neighborhoods within DC that provide the best overall value properties (highest rating at the lowest price) as well as given analysis into additional factors to look for in a listing to ensure a high-quality visit to the district. For example, we would like to discover the influence of room type in the decision of neighborhoods. And in addition, we are interested in how listing names posted by the hosts affect their Airbnbs’ values. 
...
## DC Neighborhoods With Best Overall Value
...
map <- leaflet(means_df_select) %>%
  ...
  addCircleMarkers(lng = ~longitude,
                   ...
                   label=~ID,
                   labelOptions = labelOptions(noHide = T, textOnly = T, textsize = "13px",direction = 'center',style = list(
        "color" = "white")),
                   stroke = FALSE, 
                   fillOpacity = 0.7)%>%
...
map2 <- leaflet(means_df_rt_select) %>%
  ...
  addCircleMarkers(lng = ~longitude,
                   ...
                   label=~ID,
                   labelOptions = labelOptions(noHide = T, textOnly = T, textsize = "9px",direction = 'center',style = list(
        "color" = "white")),
                   stroke = FALSE, 
                   fillOpacity = 0.6)%>% 
